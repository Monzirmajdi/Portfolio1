    ...
    function showProjectDetails(category, projectIndex) {
        ...
        let htmlContent = `
            ...
            <div class="project-images-container">
                ...
                <div class="gallery-navigation" style="display: flex; justify-content: center; gap: 10px; margin-top: 15px;">
                    <button class="nav-arrow prev-arrow" style="background: #f4c430; color: #0a0a0a; border: none; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 1.2rem;"><i class="fas fa-chevron-left"></i></button>
                    <button class="nav-arrow next-arrow" style="background: #f4c430; color: #0a0a0a; border: none; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 1.2rem;"><i class="fas fa-chevron-right"></i></button>
                </div>
        ...
        updateContentWithFade(modalGallery, htmlContent, () => {
            ...
            const prevArrow = modalGallery.querySelector(".prev-arrow");
            const nextArrow = modalGallery.querySelector(".next-arrow");
            const backToProjectsBtn = modalGallery.querySelector("#back-to-projects-btn");
            let currentImageIndex = 0;

            function updateGallery() {
                if (!imagesGrid || !progressIndicator || !project.images || project.images.length === 0) return;

                const scrollAmount = imagesGrid.children[0].offsetWidth + 15; // image width + gap
                imagesGrid.scrollTo({ left: currentImageIndex * scrollAmount, behavior: 'smooth' });
                progressIndicator.textContent = `${currentImageIndex + 1} of ${project.images.length}`;

                if (prevArrow) prevArrow.disabled = currentImageIndex === 0;
                if (nextArrow) nextArrow.disabled = currentImageIndex === project.images.length - 1;
            }
