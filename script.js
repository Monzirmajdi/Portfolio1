...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Dynamic Navbar Text based on scroll position
    const updateNavLogoText = () => {
        let currentSection = "Portfolio"; // Default to Portfolio
        sections.forEach(section => {
            const sectionTop = section.offsetTop - 150; // Adjust offset as needed
            const sectionHeight = section.clientHeight;
            if (pageYOffset >= sectionTop && pageYOffset < sectionTop + sectionHeight) {
                if (section.id === "home") currentSection = "Portfolio";
                else if (section.id === "about") currentSection = "About Me";
                else if (section.id === "portfolio") currentSection = "My Work";
                else if (section.id === "contact") currentSection = "Contact Me";
            }
        });
        navLogoSpan.textContent = currentSection;
    };

    window.addEventListener("scroll", updateNavLogoText);
    updateNavLogoText(); // Initial call to set the text on load
});
