    ...
    document.querySelectorAll(".portfolio-category").forEach(categoryDiv => {
        ...
        for (let i = 0; i < Math.min(items.length, 3); i++) {
            ...
            if (item.images && item.images.length > 0) {
                ...
                img.src = item.images[0]; // Use the first image as preview
                img.alt = item.title;
    ...
    if (data.items.length === 0) {
        ...
        data.items.forEach((item, index) => {
            ...
            projectListItem.innerHTML = `
                <img src="${item.images && item.images.length > 0 ? item.images[0] : 'images/placeholder.png'}" alt="${item.title}" class="project-preview-image">
        ...
        document.querySelectorAll(".view-project-btn").forEach(btn => {
            btn.addEventListener("click", (e) => {
                ...
                showProjectDetails(category, projectIndex);
            });
        });
    }
...
function showProjectDetails(category, projectIndex) {
    ...
    modalGallery.innerHTML = `
        ...
        <button class="btn btn-secondary" id="back-to-projects-btn">Back to Projects</button>
    `;
