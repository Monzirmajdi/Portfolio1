...
// Floating Elements Image Swap on Theme Change
document.addEventListener("DOMContentLoaded", () => {
    const floatingElements = document.querySelectorAll(".floating-elements .element");

    const imageMap = {
        "images/Untitled200_20250621030302.png": "images/Untitled200_20250621030302_black.png",
        "images/1000106782.png": "images/1000106782_black.png"
        // Assuming Untitled199_20250621025155.png is the one that stays white
    };

    function updateFloatingElements() {
        const isLightMode = document.body.classList.contains("light-mode");

        floatingElements.forEach(img => {
            const originalSrc = img.dataset.originalSrc || img.src;
            img.dataset.originalSrc = originalSrc; // Store original src

            if (imageMap[originalSrc]) {
                if (isLightMode) {
                    img.src = imageMap[originalSrc];
                } else {
                    img.src = originalSrc;
                }
            }
    ...
    const themeToggle = document.querySelector(".theme-toggle");
    if (themeToggle) {
        themeToggle.addEventListener("click", updateFloatingElements);
    }
});
